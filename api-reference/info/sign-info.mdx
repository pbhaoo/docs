...
- 目前除了公共接口(接口有说明不需签名)，所有其它接口都规定需要进行签名验证，以保证请求数据不会被恶意篡改；
- 需要签名的接口，生成的签名在请求头中以字段`Signature`的值进行传递；
- 所有签名接口都需要在请求参数中带上参数：`timestamp` ，其值应当是请求发送时刻的UNIX时间戳（毫秒）。服务器收到请求时会判断请求中的时间戳，如果是`5000`毫秒之前发出的，则请求会被认为无效。
